...
which a helper is provided in the Weave script:
    ...
    $ docker run --net=weave -h foo.weave.local $(weave dns-args) -tdi alpine:latest
    $ docker run --net=weave -h bar.weave.local $(weave dns-args) -ti alpine:latest
    # ping foo
...
then run your containers using the Docker command-line:
    ...
    host1$ docker run --net=weave -ti alpine:latest
    root@1458e848cd90:/# hostname -i
    10.32.0.2

    host2$ docker run --net=weave -ti alpine:latest
