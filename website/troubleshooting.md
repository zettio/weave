...
[How Weave Net Works]({{ '/concepts/how-it-works' | relative_url }}).
...
[encryption]({{ '/concepts/encryption' | relative_url }}) is in use for communication
between peers.

 * **PeerDiscovery** - indicates whether
[automatic peer discovery]({{ '/tasks/ipam/allocation-multi-ipam' | relative_url }}) is
...
{% raw %}    $ weave report -f '{{.DNS.Domain}}' weave.local.{% endraw %}
...
Proxy]({{ '/tasks/weave-docker-api/weave-docker-api' | relative_url }}) then
...
instructions]({{ '/install/installing-weave' | relative_url }}).
